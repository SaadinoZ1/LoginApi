...
namespace LoginApiApp.Services
{
    public class ApiService
    {
        ...
        public async Task<bool> RegisterAsync(string username, string password)
        {
            ...
            var response = await _httpClient.PostAsync("http://192.168.4.219:5178/api/Auth/register", content);
        ...
        public async Task<UserDto?> LoginAsync(string username, string password)
        {
            try
            {
                ...
                var loginUrl = "http://192.168.4.219:5178/api/Auth/login";
        ...
        public async Task<string?> RefreshTokenAsync(string refreshToken)
        {
            ...
            var response = await _httpClient.PostAsync("http://192.168.4.219:5178/api/Auth/refresh", content);
        ...
        {
            var url = "http://192.168.4.219:5178/api/Trajet";
        ...
        {
            var response = await _httpClient.GetAsync($"http://192.168.4.219:5178/api/Trajet/{id}");
            if (response.IsSuccessStatusCode)
            {
        ...
        public async Task<bool> AddTrajetAsync(TrajetDto trajetDto)
        {
            ...
            var response = await _httpClient.PostAsync("http://192.168.4.219:5178/api/Trajet", content);
            return response.IsSuccessStatusCode;
        }
        ...
        {
            var content = new StringContent(JsonConvert.SerializeObject(trajetDto), Encoding.UTF8, "application/json");
            var response = await _httpClient.PutAsync($"http://192.168.4.219:5178/api/Trajet/{id}", content);
            return response.IsSuccessStatusCode;
        }

        public async Task<bool> DeleteTrajetAsync(int trajetId)
        {
            var response = await _httpClient.DeleteAsync($"http://192.168.4.219:5178/api/Trajet/{trajetId}");
            return response.IsSuccessStatusCode;
        }
        ...
        {
            var response = await _httpClient.GetAsync($"http://192.168.4.219:5178/api/Vehicule/{id}");
            if (response.IsSuccessStatusCode)
            {
                var jsonResponse = await response.Content.ReadAsStringAsync();
        ...
        public async Task<bool> AddVehiculeAsync(VehiculeDto vehiculeDto)
        {
            ...
            var response = await _httpClient.PostAsync("http://192.168.4.219:5178/api/Vehicule", content);
            return response.IsSuccessStatusCode;
        }
        public async Task<bool> EditVehiculeAsync(int id, VehiculeDto vehiculeDto)
        {
            var content = new StringContent(JsonConvert.SerializeObject(vehiculeDto), Encoding.UTF8, "application/json");
            var response = await _httpClient.PutAsync($"http://192.168.4.219:5178/api/Vehicule/{id}", content);
            return response.IsSuccessStatusCode;
        }
        public async Task<bool> DeleteVehiculeAsync(int vehiculeId)
        {
            var response = await _httpClient.DeleteAsync($"http://192.168.4.219:5178/api/Vehicule/{vehiculeId}");
            return response.IsSuccessStatusCode;
        }
    }
}
